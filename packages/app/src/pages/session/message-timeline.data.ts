...
export namespace Timeline {
	export function constructMessageRows(
		...
		if (!userMessage) return rows

		const previousUserMessage = index > 0
		const userParts = (getMessageParts(userMessage.id) ?? []).filter(Boolean)
		const comments = userParts.flatMap((p) => MessageComment.fromPart(p) ?? [])
		const compaction = userParts.some((p) => p.type === "compaction")
		const interruptedMessageIndex = (assistantMessages ?? [])
			.filter(Boolean)
			.findIndex((m) => m.error?.name === "MessageAbortedError")
		const interrupted = interruptedMessageIndex !== -1
		const error = (assistantMessages ?? [])
			.filter(Boolean)
			.find((m) => m.error && m.error.name !== "MessageAbortedError")?.error

		const assistantPartRefs = (assistantMessages ?? []).filter(Boolean).flatMap((message, messageIndex) =>
		...
		if (isActive && status === "busy" && !error && (showReasoning ? assistantPartRefs.length === 0 : true)) {
			const heading = (assistantMessages ?? [])
				.filter(Boolean)
				.flatMap((message) => (getMessageParts(message.id) ?? []).filter(Boolean))
		...
		const diffs = (() => {
			const arr = userMessage.summary?.diffs ?? []
			const seen = new Set<string>()
			const result: SummaryDiff[] = []
			for (let i = arr.length - 1; i >= 0; i--) {
				const d = arr[i]
				if (!isSummaryDiff(d)) continue
	...
	function isSummaryDiff(value: SnapshotFileDiff): value is SummaryDiff {
		return value != null && typeof value.file === "string"
	}
